...
class App extends Component {
  ...
  componentDidMount() {
    ...
    loadMapJS(
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyBCWKeNDf3lu3dOdl0cldnphsy7LDZyVgU&callback=initMap"
    );
  }
  ...
  initMap() {
    ...
    var map = new window.google.maps.Map(mapview, {
      center: { lat: 41.939891, lng: 12.528062 },
      zoom: 15,
    ...
    fetch(url)
      .then(function(response) {
        if (response.status !== 200) {
          self.state.infowindow.setContent("Error, data can't be loaded");
          return;
        }
